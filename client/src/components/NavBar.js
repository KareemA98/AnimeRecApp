...
const NavBar = ({ anime, tag, doAuth, loggedIn, hideWatched, alterState, logOut }) => {
  ...
  return (
    <Flex bg="gray.500" justify="flex-end" padding={1}>
      ...
      {loggedIn ?
        <>
          <Checkbox isChecked={hideWatched} onChange={alterState}>Hide Watched</Checkbox>
          <Text m={2} fontSize="30px">Logged In</Text>
          <Button m={2} onClick={logOut}>Logout of MAL</Button>
        </> :
